...
async function downloadContent(tab, incognito, message) {
	try {
		...
		if (message.saveWithWebDAV) {
			...
			response = await saveWithWebDAV(message.taskId, encodeSharpCharacter(message.filename), pageContent, message.webDAVURL, message.webDAVUser, message.webDAVPassword, message.filenameConflictAction);
			...
			const pageContent = await (await fetch(message.content)).text();
			response = await saveToGitHub(message.taskId, encodeSharpCharacter(message.filename), pageContent, message.githubToken, message.githubUser, message.githubRepository, message.githubBranch, message.filenameConflictAction);
