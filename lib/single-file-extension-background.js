(function () {
	...
	async function sendResponse(tabId, requestId, response) {
		for (let blockIndex = 0; blockIndex * MAX_CONTENT_SIZE$1 <= response.array.length; blockIndex++) {
			const message = {
			...
			if (message.truncated) {
				message.finished = (blockIndex + 1) * MAX_CONTENT_SIZE$1 > response.array.length;
				message.array = response.array.slice(blockIndex * MAX_CONTENT_SIZE$1, (blockIndex + 1) * MAX_CONTENT_SIZE$1);
			} else {
	...
	async function onTabReplaced$2(addedTabId, removedTabId) {
	...
	async function onMessage$8(message, sender) {
		if (message.method.endsWith(".init")) {
			const options = await [getOptions(sender.tab.url)];
	...
	function onTabReplaced$1(addedTabId, removedTabId) {
	...
	async function downloadTabPage(message, tab) {
		if (message.openEditor) {
			onEdit(tab.id);
			await open({ tabIndex: tab.index + 1, filename: message.filename, content: await (await fetch(message.content)).text() });
		} else {
